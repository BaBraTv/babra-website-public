...
export function evaluateProductionEnvironment(environment) {
  const withdrawalsDisabled = environment.AFFILIATE_WITHDRAWALS_ENABLED === "false";
  const missing = requiredProductionVariables.filter((name) => !environment[name] && !(withdrawalsDisabled && name === "AFFILIATE_WITHDRAWAL_MIN_MINOR"));
  ...
  const affiliatePolicyValid = withdrawalsDisabled || Number.isSafeInteger(minimum) && minimum > 0 && (maximum == null || Number.isSafeInteger(maximum) && maximum >= minimum);
  ...
  return {
    ...
    checks: {
      ...
      runtimeIdentityValid, migrationIdentityValid, affiliatePolicyValid, withdrawalsDisabled, appUrlsHttps: appUrlsValid
    }
  };
}
